...
- [ ] 4.1 实现App组件核心状态
  ...
  - 加载配置（使用LocalJsonDataProvider）
  - 从localStorage恢复状态（使用StorageManager.load）
  - 处理配置加载错误（显示错误页面+重试按钮）
  - **验收**: 应用启动，配置加载成功，状态初始化正确
  - _Requirements: 11.1-11.4, 9.2_
...
- [ ] 4.3 实现装备装配逻辑
  ...
  - **卸载逻辑**（selectedItemId === null且槽位有装备）:
    ...
    - 保存到localStorage
  - **验收**: 装配/卸载逻辑正确，toast提示正确，selectedItemId清除时机正确
...
- [ ] 7.2 连接所有组件和状态
  - 将App状态传递到CharacterView（equippedItems, selectedItemId）
  - 将App状态传递到EquipmentLibrary（selectedItemId）
  - 将回调函数传递到子组件（onItemSelect, onSlotClick）
  - 将StatusBar连接到状态（equippedItems, slots）
  - 实现Toast显示逻辑（错误时显示）
  - **验收**: 所有组件正确连接，交互流程完整
...
- [ ] 9.1 核心功能验证
  ...
  - ✓ 显示装备库（至少12件装备）
  - ✓ 点击装备，高亮兼容槽位，灰化不兼容槽位
  - ✓ 点击兼容槽位，装备成功装配，选中状态清除
  - ✓ 点击不兼容槽位，显示toast "此装备无法装配到该槽位"，选中状态保留
  - ✓ 点击已满槽位，显示toast "该槽位已满（最多X件）"，选中状态保留
  - ✓ 点击已装配槽位（无选中装备），卸载最后一个装备
  - ✓ StatusBar显示正确计数："已装配: X/Y 件装备 (必选: M/N)"
  - ✓ 头盔和护肋都装配后，显示完整性指示器（绿色✓）
  - ✓ 刷新页面，装配状态正确恢复
  - ✓ 点击Reset，清空所有装备和localStorage
  - ✓ 响应式：缩放窗口，布局正确切换（768px断点）
  - ✓ 响应式：槽位位置和尺寸按百分比缩放
...
- [ ] 10.1 应用Pixelact UI组件
  - 按需引入Pixelact UI组件（Button, Card等）
  - 如组件缺失，使用shadcn/ui + 自定义像素边框
...
- [ ]* 12.1 组件单元测试
  - Toast组件：显示/隐藏，自动关闭
  - StatusBar组件：计数公式，完整性指示器，Reset按钮
  - EquipmentCard组件：点击选择，选中状态
  - EquipmentSlot组件：视觉状态，点击装配/卸载
